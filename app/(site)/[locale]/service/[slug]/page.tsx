...
import { getCompany, getServices } from "@/lib/cms/site-data";
...
export default async function ServicePage({
  ...
  const { slug } = await params;
  const [services, company, t, tc] = await Promise.all([
    getServices(),
    getCompany(),
    getTranslations("service"),
    getTranslations("common"),
  ]);
  const idx = services.findIndex((item) => item.slug === slug);
  const service = services[idx];
  if (!service) notFound();
  const related = services.filter((item) => item.slug !== service.slug).slice(0, 3);
  // The service's own photo wins; otherwise fall back to a rotating stock shot.
  const image = service.image || featureImages[idx % featureImages.length];
      ...
      <main>
          ...
          <div className="container-x grid gap-12 lg:grid-cols-3 lg:gap-12">
            <div className="lg:col-span-2">
              ...
              <Reveal delay={0.1}>
                ...
                <h3 className="mt-10">{t("whatWeDeliver")}</h3>
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                <div className="rounded-card bg-charcoal p-7 text-sand">
                  ...
                  <h3 className="mt-4 text-sand">{t("quoteTitle")}</h3>
                  <p className="mt-2 text-sm text-mist">{t("quoteBody")}</p>
                  <Link
                    href="/contact"
                ...
                <div className="rounded-card border border-line-light bg-white p-6">
                  <p className="eyebrow mb-3">{t("otherServices")}</p>
        ...
        <section className="section bg-white text-ink">
          <div className="container-x">
            <h2 className="mb-10">{t("relatedServices")}</h2>
