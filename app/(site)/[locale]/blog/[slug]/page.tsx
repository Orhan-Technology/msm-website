...
import { getPosts, getTeam } from "@/lib/cms/site-data";
...
export default async function BlogPostPage({
  ...
  const { slug } = await params;
  const [posts, team, t] = await Promise.all([getPosts(), getTeam(), getTranslations("blog")]);
      ...
      <main>
        ...
        <section className="bg-charcoal text-sand">
          <div className="container-x pb-12 pt-36 md:pt-40">
            ...
            >
              <ArrowLeft className="h-4 w-4 rtl:-scale-x-100" /> {t("backToBlog")}
            </Link>
            ...
            <p className="mt-4 text-sm text-mist">
              {fmtDate(post.date)} · {t("by")} {post.author}
            </p>
          </div>
            ...
            <aside className="space-y-6">
              {author && (
                <div className="rounded-card border border-line-light bg-white p-6">
                  <p className="eyebrow mb-4">{t("author")}</p>
              ...
              <div className="rounded-card bg-charcoal p-6 text-sand">
                ...
                <p className="mb-4 text-sm text-mist">{t("newsletterBody")}</p>
                <NewsletterForm />
              </div>
            </aside>
          </div>
        ...
        <section className="section bg-white text-ink">
          <div className="container-x">
            <h2 className="mb-10">{t("relatedArticles")}</h2>
